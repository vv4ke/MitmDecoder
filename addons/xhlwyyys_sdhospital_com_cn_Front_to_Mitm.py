...
Run as follows:
mitmweb.exe -s .\addons\addons_jkgz_hn165_com_Front_and_Mitm.py --listen-port 8081 --web-port 8082 --mode upstream:http://127.0.0.1:8080 -k
"""
...
class Front_and_Mitm:
    def __init__(self):
        self.request_judge_keyword = 'encryption_data'
        self.request_regex = r'{\"encryption_data\":\"(.+)\"}'
        self.mark_header = 'Vvak4'

        self.response_template = '{"encryption_response":"%s"}'
    ...
    def request(self, flow):
        try:
            ...
            if self.request_judge_keyword not in request_body:
                ctx.log.error(f"未在流向中端的 {flow.request.pretty_url} 请求数据包中匹配到特征关键字 {self.request_judge_keyword},故选择pass")
                return

            # step2：匹配报文中的密文
            if not re.search(self.request_regex, request_body):
                ctx.log.error(f"在流向中端的 {flow.request.pretty_url} 请求数据包中匹配到特征关键字 {self.request_judge_keyword},但是未匹配出需要解密的密文,故选择pass")
                return
